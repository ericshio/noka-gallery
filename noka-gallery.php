...
class Noka_Gallery {
    ...
    public function add_shortcode_column( $columns ) {
        // Insert Shortcode column after Title
        $new_columns = array();
    ...
    public function register_cpt() { 
        register_post_type( 'noka_gallery', array( 
            // RENAMED: "Noka Gallery" (singular for menu)
    ...
    public function get_gallery_data( $data ) { $post_id = $data['id']; $ids_str = get_post_meta( $post_id, '_noka_gallery_ids', true ); $images = []; if(!empty($ids_str)) { $ids = explode(',', $ids_str); foreach($ids as $media_id) { $url = wp_get_attachment_image_url($media_id, 'large'); if(!$url) continue; $images[] = ['id' => $media_id, 'url' => $url]; } } $settings = array( 'cols_d' => get_post_meta( $post_id, '_noka_cols_d', true ) ?: 3, 'cols_t' => get_post_meta( $post_id, '_noka_cols_t', true ) ?: 2, 'cols_m' => get_post_meta( $post_id, '_noka_cols_m', true ) ?: 1, 'gap' => get_post_meta( $post_id, '_noka_gap', true ) ?: 10, 'radius' => get_post_meta( $post_id, '_noka_radius', true ) ?: 0, ); return array('images' => $images, 'settings' => $settings); }
    ...
    public function render_main_meta_box( $post ) { $gallery_ids = get_post_meta( $post->ID, '_noka_gallery_ids', true ); $image_size = get_post_meta( $post->ID, '_noka_image_size', true ) ?: 'large'; $cols_d = get_post_meta( $post->ID, '_noka_cols_d', true ) ?: 3; $cols_t = get_post_meta( $post->ID, '_noka_cols_t', true ) ?: 2; $cols_m = get_post_meta( $post->ID, '_noka_cols_m', true ) ?: 1; $gap = get_post_meta( $post->ID, '_noka_gap', true ) ?: 10; $lightbox = get_post_meta( $post->ID, '_noka_lightbox', true ); if($lightbox === '') $lightbox = '1'; $hover_anim = get_post_meta( $post->ID, '_noka_hover_anim', true ) ?: 'none'; $show_overlay = get_post_meta( $post->ID, '_noka_show_overlay', true ) ?: '0'; $overlay_bg = get_post_meta( $post->ID, '_noka_overlay_bg', true ) ?: 'rgba(0,0,0,0.5)'; $cursor = get_post_meta( $post->ID, '_noka_cursor', true ) ?: 'default'; $radius = get_post_meta( $post->ID, '_noka_radius', true ) ?: '0'; $lightbox_bg = get_post_meta( $post->ID, '_noka_lightbox_bg', true ) ?: 'rgba(0,0,0,0.85)'; require NOKA_PATH . 'includes/admin-view.php'; }
    public function save_gallery_data( $post_id ) { if ( isset( $_POST['noka_gallery_ids'] ) ) update_post_meta( $post_id, '_noka_gallery_ids', sanitize_text_field( $_POST['noka_gallery_ids'] ) ); $int_fields = ['noka_cols_d', 'noka_cols_t', 'noka_cols_m', 'noka_gap', 'noka_radius']; foreach($int_fields as $f) if(isset($_POST[$f])) update_post_meta( $post_id, "_$f", intval($_POST[$f]) ); update_post_meta( $post_id, '_noka_lightbox', isset($_POST['noka_lightbox']) ? '1' : '0' ); update_post_meta( $post_id, '_noka_show_overlay', isset($_POST['noka_show_overlay']) ? '1' : '0' ); if(isset($_POST['noka_image_size'])) update_post_meta( $post_id, '_noka_image_size', sanitize_text_field($_POST['noka_image_size']) ); if(isset($_POST['noka_hover_anim'])) update_post_meta( $post_id, '_noka_hover_anim', sanitize_text_field($_POST['noka_hover_anim']) ); if(isset($_POST['noka_overlay_bg'])) update_post_meta( $post_id, '_noka_overlay_bg', sanitize_text_field($_POST['noka_overlay_bg']) ); if(isset($_POST['noka_cursor'])) update_post_meta( $post_id, '_noka_cursor', sanitize_text_field($_POST['noka_cursor']) ); if(isset($_POST['noka_lightbox_bg'])) update_post_meta( $post_id, '_noka_lightbox_bg', sanitize_text_field($_POST['noka_lightbox_bg']) ); }
    public function admin_scripts( $hook ) { global $post; if ( ($hook == 'post-new.php' || $hook == 'post.php') && 'noka_gallery' === $post->post_type ) { wp_enqueue_media(); wp_enqueue_style( 'wp-color-picker' ); wp_enqueue_script( 'jquery-ui-sortable' ); wp_enqueue_script( 'noka-admin', NOKA_URL . 'includes/assets/js/admin.js', array( 'jquery', 'jquery-ui-sortable', 'wp-color-picker' ), NOKA_VERSION, true ); wp_enqueue_style( 'noka-admin-css', NOKA_URL . 'includes/assets/css/admin.css', array(), NOKA_VERSION ); } }
